...
async function crearOfertaTrabajo(data, adminId) {
    try {
        const adminExistente = await Usuario.findById(adminId);
...
async function obtenerTodasOfertasTrabajo() {
    try {
        ...
        console.error('Error al obtener todas las ofertas de trabajo:', error.message);
        throw error;
    }
}
